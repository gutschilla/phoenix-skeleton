...
(function() {
  ...
  startServer = function(config, callback) {
    var child, commandComponents, customServerTimeout, error, opts, port, publicPath, server, serverCb, serverConfig, serverOpts;
    if (callback == null) {
      callback = function() {};
    }
    ...
    if (config.server.path) {
      ...
      if (server.startServer.length === 2) {
        return server.startServer(serverConfig, serverCb);
      } else {
        return server.startServer(port, publicPath, serverCb);
      }
  ...
  initWatcher = function(config, callback) {
    var allConfigFiles, bower, component, exists, getFiles, ref, watched;
    allConfigFiles = config._normalized.paths.allConfigFiles;
    ref = config._normalized.packageInfo, bower = ref.bower, component = ref.component;
    getFiles = function(pkgs) {
      return [].concat.apply([], pkgs.components.map(function(_) {
        return _.files;
      }));
    };
    watched = config.paths.watched.concat(allConfigFiles, getFiles(bower), getFiles(component));
  ...
  generateCompilationLog = function(startTime, allAssets, generatedFiles, disposedFiles) {
    var assetsLog, cachedCount, cachedCountName, cachedLog, compiled, compiledCount, compiledLog, copied, copiedCount, disposedCount, generated, generatedLog, getName, main, nonAssetsLog, noun, sep;
  ...
  initialize = function(options, configParams, onCompile, callback) {
    return helpers.loadConfig(options.config, configParams, function(error, config) {
      var alwaysEnabled, callCompileCallbacks, callbacks, compilers, fileList, joinConfig, launchWatcher, linters, optimizers, packages, plugins, server, unfiltered;
